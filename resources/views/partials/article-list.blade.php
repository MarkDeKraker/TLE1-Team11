            ...
            <div class="p-2">
                <div class="pt-2">
                    @foreach($article->ages as $age)
                        <span class="rounded-full bg-orange-500 text-white text-xs px-2 py-1 inline-flex mr-2 mb-2">
                            {{ $age->age }}
                        </span>
                    @endforeach
                    @foreach($article->subjects as $subject)
                        <span class="rounded-full bg-white border text-xs border-orange-500 px-2 py-1 inline-flex mr-2 mb-2">
                            {{ $subject->subject }}
                        </span>
                    @endforeach
                </div>
